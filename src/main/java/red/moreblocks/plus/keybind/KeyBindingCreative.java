...
package red.moreblocks.plus.keybind;

import red.moreblocks.plus.procedure.ProcedureCreativeOnKey;
import red.moreblocks.plus.MbPlusMod;
...
public class KeyBindingCreative extends ElementsMbPlusMod.ModElement {
	private KeyBinding keys;
	public KeyBindingCreative(ElementsMbPlusMod instance) {
		super(instance, 200);
	}
	...
	public void init(FMLInitializationEvent event) {
		keys = new KeyBinding("key.mcreator.creative", Keyboard.KEY_F7, "key.categories.misc");
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (Minecraft.getMinecraft().currentScreen == null) {
			if (org.lwjgl.input.Keyboard.isKeyDown(keys.getKeyCode())) {
				MbPlusMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
	...
	private static void pressAction(EntityPlayer entity) {
			...
			return;
		{
			...
			ProcedureCreativeOnKey.executeProcedure($_dependencies);
		}
	}
}
